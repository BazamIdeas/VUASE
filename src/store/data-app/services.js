export const servicesHardcoded = {
  'logo-empresa': {
    list: ['Prueba 3', 'Prueba 4'],
    title: 'LOGO PARA EMPRESA',
    description: `Lorem Ipsum is simply dummy text of the printing and typesetting industry Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an  unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged`,
    icon: '/icons/logo_a_medida_hover.svg',
    url: '/',
    whatWeOffer: [
      {
        title: 'Hola',
        description: 'Descripción',
        icon: '/icons/logo_a_medida_hover.svg'
      }, {
        title: 'Hola',
        description: 'Descripción',
        icon: '/icons/logo_a_medida_hover.svg'
      }, {
        title: 'Hola',
        description: 'Descripción',
        icon: '/icons/logo_a_medida_hover.svg'
      }, {
        title: 'Hola',
        description: 'Descripción',
        icon: '/icons/logo_a_medida_hover.svg'
      }, {
        title: 'Hola',
        description: 'Descripción',
        icon: '/icons/logo_a_medida_hover.svg'
      }, {
        title: 'Hola',
        description: 'Descripción',
        icon: '/icons/logo_a_medida_hover.svg'
      }
    ],
    whatYouGet: {
      img: '../images/pages/mockup-ferreteria-flores.png',
      columns: [
        [{
          title: 'Hola',
          items: ['prueba', 'prueba 2', 'prueba', 'prueba 2', 'prueba', 'prueba 2'],
          icon: '/icons/logo_a_medida_hover.svg',
          description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla eu mi pharetra massa porttitor sollicitudin. Quisque vulputate pellentesque est, id ullamcorper metus elementum nec. Pellentesque quis magna quam.'
        }]
      ]
    },
    frequentQuestions: [
      {
        question: '¿Que es la vida?',
        response: 'La vida es algo incomprensible'
      }, {
        question: '¿Por que eva comio la fruta prohibida?',
        response: 'Por vagabunda'
      }, {
        question: '¿Que logo se adapta a mi pensamiento?',
        response: 'La vida es algo incomprensible'
      }, {
        question: '¿Que es la vida?',
        response: 'La vida es algo incomprensible'
      }, {
        question: '¿Por que eva comio la fruta prohibida?',
        response: 'Por vagabunda'
      }, {
        question: '¿Que logo se adapta a mi pensamiento?',
        response: 'La vida es algo incomprensible'
      }
    ],
    testimonies: [{
      stars: 4,
      logo: 'papeleria.svg',
      description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla eu mi pharetra massa porttitor sollicitudin.'
    }],
    topSlider: [
      {
        url: '/images/pages/rediseno-acc-group.png',
        color: 'rgb(0, 78, 126)'
      }
    ],
    creativeProcess: [
      {
        src: '/images/carousels/creative-process-1.png',
        alt: 'creative-process-1'
      },
      {
        src: '/images/carousels/creative-process-2.png',
        alt: 'creative-process-2'
      },
      {
        src: '/images/carousels/creative-process-3.png',
        alt: 'creative-process-3'
      }
    ],
    steps: [
      {
        title: '1. BRIEF Y SELECTOR',
        color: '#323c9e',
        icon: '/icons/creative-process-1.svg',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec quis diam ut dui commodo finibus. Sed at metus condimentum, mollis justo eget, venenatis metus. In eu mi fringilla, pharetra purus ac, venenatis nisi. ',
        alt: 'brief-y-selector'
      },
      {
        title: '2. CONCEPTOS',
        color: '#1d8127',
        icon: '/icons/creative-process-2.svg',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec quis diam ut dui commodo finibus. Sed at metus condimentum, mollis justo eget, venenatis metus. In eu mi fringilla, pharetra purus ac, venenatis nisi. ',
        alt: 'conceptos'
      },
      {
        title: '3. ENTREGA Y DOCUMENTACIÓN',
        color: '#c3953f',
        icon: '/icons/creative-process-3.svg',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec quis diam ut dui commodo finibus. Sed at metus condimentum, mollis justo eget, venenatis metus. In eu mi fringilla, pharetra purus ac, venenatis nisi. ',
        alt: 'entrega-y-documentacion'
      }
    ],
    carousel: {
      items1: [
        { img: '/images/examples/1.png' },
        { img: '/images/examples/2.png' },
        { img: '/images/examples/3.png' },
        { img: '/images/examples/4.png' },
        { img: '/images/examples/1.png' },
        { img: '/images/examples/2.png' },
        { img: '/images/examples/3.png' },
        { img: '/images/examples/4.png' },
        { img: '/images/examples/1.png' },
        { img: '/images/examples/2.png' },
        { img: '/images/examples/3.png' },
        { img: '/images/examples/4.png' },
        { img: '/images/examples/1.png' }
      ],
      items2: [
        { img: '/images/examples/5.png' },
        { img: '/images/examples/6.png' },
        { img: '/images/examples/7.png' },
        { img: '/images/examples/8.png' },
        { img: '/images/examples/5.png' },
        { img: '/images/examples/6.png' },
        { img: '/images/examples/7.png' },
        { img: '/images/examples/8.png' },
        { img: '/images/examples/5.png' },
        { img: '/images/examples/6.png' },
        { img: '/images/examples/7.png' },
        { img: '/images/examples/8.png' }
      ]
    }
  },
  // Redes Sociales
  'perfiles-de-redes-sociales': {
    list: ['Prueba 3', 'Prueba 4'],
    description: `Lorem Ipsum is simply dummy text of the printing and typesetting industry Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an  unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged`,
    icon: '/icons/perfiles_de_redes_sociales_2.svg',
    url: '/',
    whatWeOffer: [
      {
        title: 'Hola',
        description: 'Descripción',
        icon: '/icons/logo_a_medida_hover.svg'
      }, {
        title: 'Hola',
        description: 'Descripción',
        icon: '/icons/logo_a_medida_hover.svg'
      }, {
        title: 'Hola',
        description: 'Descripción',
        icon: '/icons/logo_a_medida_hover.svg'
      }, {
        title: 'Hola',
        description: 'Descripción',
        icon: '/icons/logo_a_medida_hover.svg'
      }, {
        title: 'Hola',
        description: 'Descripción',
        icon: '/icons/logo_a_medida_hover.svg'
      }, {
        title: 'Hola',
        description: 'Descripción',
        icon: '/icons/logo_a_medida_hover.svg'
      }
    ],
    whatYouGet: [
      [{
        title: 'Hola',
        items: ['prueba', 'prueba 2', 'prueba', 'prueba 2', 'prueba', 'prueba 2'],
        icon: '/icons/logo_a_medida_hover.svg',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla eu mi pharetra massa porttitor sollicitudin. Quisque vulputate pellentesque est, id ullamcorper metus elementum nec. Pellentesque quis magna quam.'
      }]
    ],
    frequentQuestions: [{
      question: '¿Que es la vida?',
      response: 'La vida es algo incomprensible'
    }, {
      question: '¿Por que eva comio la fruta prohibida?',
      response: 'Por vagabunda'
    }, {
      question: '¿Que logo se adapta a mi pensamiento?',
      response: 'La vida es algo incomprensible'
    }, {
      question: '¿Que es la vida?',
      response: 'La vida es algo incomprensible'
    }, {
      question: '¿Por que eva comio la fruta prohibida?',
      response: 'Por vagabunda'
    }, {
      question: '¿Que logo se adapta a mi pensamiento?',
      response: 'La vida es algo incomprensible'
    }],
    testimonies: [{
      stars: 4,
      logo: 'papeleria.svg',
      description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla eu mi pharetra massa porttitor sollicitudin.'
    }],
    topSlider: [
      {
        url: '/images/pages/rediseno-acc-group.png',
        color: 'rgb(0, 78, 126)'
      }
    ],
    creativeProcess: [
      {
        src: '/images/carousels/creative-process-1.png',
        alt: 'creative-process-1'
      },
      {
        src: '/images/carousels/creative-process-2.png',
        alt: 'creative-process-2'
      },
      {
        src: '/images/carousels/creative-process-3.png',
        alt: 'creative-process-3'
      }
    ],
    steps: [
      {
        title: '1. BRIEF Y SELECTOR',
        color: '#323c9e',
        icon: '/icons/creative-process-1.svg',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec quis diam ut dui commodo finibus. Sed at metus condimentum, mollis justo eget, venenatis metus. In eu mi fringilla, pharetra purus ac, venenatis nisi. ',
        alt: 'brief-y-selector'
      },
      {
        title: '2. CONCEPTOS',
        color: '#1d8127',
        icon: '/icons/creative-process-2.svg',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec quis diam ut dui commodo finibus. Sed at metus condimentum, mollis justo eget, venenatis metus. In eu mi fringilla, pharetra purus ac, venenatis nisi. ',
        alt: 'conceptos'
      },
      {
        title: '3. ENTREGA Y DOCUMENTACIÓN',
        color: '#c3953f',
        icon: '/icons/creative-process-3.svg',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec quis diam ut dui commodo finibus. Sed at metus condimentum, mollis justo eget, venenatis metus. In eu mi fringilla, pharetra purus ac, venenatis nisi. ',
        alt: 'entrega-y-documentacion'
      }
    ]
  },
  // ROTULOS
  'rotulo-vehicular': {
    list: ['Prueba 3', 'Prueba 4'],
    description: `Lorem Ipsum is simply dummy text of the printing and typesetting industry Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an  unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged`,
    icon: '/icons/rotulo_vehicular.svg',
    url: '/'
  },
  'rotulo-local': {
    list: ['Prueba 3', 'Prueba 4'],
    description: `Lorem Ipsum is simply dummy text of the printing and typesetting industry Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an  unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged`,
    icon: '/icons/logo_a_medida_hover.svg',
    url: '/'
  },
  'rotulo-banner': {
    list: ['Prueba 3', 'Prueba 4'],
    description: `Lorem Ipsum is simply dummy text of the printing and typesetting industry Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an  unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged`,
    icon: '/icons/logo_a_medida_hover.svg',
    url: '/'
  },
  'rotulo-furgoneta-camion': {
    list: ['Prueba 3', 'Prueba 4'],
    description: `Lorem Ipsum is simply dummy text of the printing and typesetting industry Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an  unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged`,
    icon: '/icons/logo_a_medida_hover.svg',
    url: '/'
  },
  'rotulo-etiqueta-packaging': {
    list: ['Prueba 3', 'Prueba 4'],
    description: `Lorem Ipsum is simply dummy text of the printing and typesetting industry Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an  unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged`,
    icon: '/icons/logo_a_medida_hover.svg',
    url: '/'
  },
  'rotulo-vestimenta-uniforme': {
    list: ['Prueba 3', 'Prueba 4'],
    description: `Lorem Ipsum is simply dummy text of the printing and typesetting industry Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an  unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged`,
    icon: '/icons/logo_a_medida_hover.svg',
    url: '/'
  }
}
