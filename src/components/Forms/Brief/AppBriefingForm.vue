<template>
  <form id="app">
    <v-layout row wrap class="px-2 mt-4 mb-5">
      ...
      <v-flex md8>
        <v-text-field name="email" v-model="formFields.email" label="Correo electronico" v-validate="'required|email'" :error-messages="errors.collect('email')" box></v-text-field>
        <v-text-field name="names" v-model="formFields.names" label="Nombres" v-validate="'required'" :error-messages="errors.collect('names')" box></v-text-field>
        <v-text-field name="phone" v-model="formFields.phone" label="Teléfono" v-validate="'required'" :error-messages="errors.collect('phone')" box></v-text-field>
        <v-text-field name="company" v-model="formFields.company" label="Nombre de la empresa" v-validate="'required'" :error-messages="errors.collect('company')" box></v-text-field>
      </v-flex>
      ...
      <v-flex md8>
        <v-text-field v-for="field in form" :key="field.name" v-if="field.type === 'text'" :ref="field.name" v-model="formFields[field.name]" :label="field.label" box></v-text-field>
        <v-textarea v-else-if="field.type === 'textarea'" :ref="field.name" v-model="formFields[field.name]" :label="field.label" box></v-textarea>
        <div v-else-if="field.type === 'checkbox'" class="my-3">
          <v-layout row wrap>
            ...
            <v-flex md6 v-for="option in field.options" :key="option">
              <v-checkbox :label="option" v-model="formFields[field.name]" :value="option"></v-checkbox>
            </v-flex>
          </v-layout>
        </div>
            ...
            <v-flex md12>
              ...
              <v-input prepend-icon="folder_open" :append-icon="formFields[field.name] ? 'close' : ''" @click:append="clearFile(field.name)" class="file my-3">
                ...
                <p style="margin-bottom: 0; padding: 2px 16px; font-weight: 500; color: #737373;border: 2px solid #929292; border-radius: 5px; cursor: pointer">
                  <label :for="field.name" v-if="!formFields[field.name]" style="cursor: pointer">Seleccionar Archivo </label>
                  <label :for="field.name" v-if="formFields[field.name]" style="cursor: pointer">{{ formFields[field.name].name }}</label>
                </p>
              </v-input>
            </v-flex>
          </v-layout>
  ...
  export default {
    ...
    created () {
      ...
      if (vue.form) {
        vue.form.forEach(field => {
          if (field.type === 'text' || field.type === 'textarea') vue.$set(vue.formFields, field.name, '')
          if (field.type === 'checkbox') vue.$set(vue.formFields, field.name, [])
          if (field.type === 'file') vue.$set(vue.formFields, field.name, null)
        })
      }
    },
    computed: {
    ...
    methods: {
      setFile (name, event) {
        ...
        this.formFields[name] = event.target.files[0]
      },
      clearFile (name) {
      ...
      submit: function (newVal, oldVal) {
        ...
        this.$validator.validate().then(async function (valid) {
          if (valid) {
            ...
            brief.information = this.formFields
