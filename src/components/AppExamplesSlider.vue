    ...
    <v-layout row class="ma-0" style="position:relative">
      <v-flex xs12 class="examples-slider-container pa-0" ref="examplesSliderContainer" :class="{'shadow': shadow}" >
        <div class="examples-slider-scrollable" ref="examplesSliderScrollable" data-scrollable @mouseover="scroll" @mouseout="clearScroll">
          <v-layout row>
            <v-flex @click="go(example.url)" v-for="(example, i) in examples" :key="i" class="example-slider-item" align-content-center @mouseover="hovering; inHover = i" @mouseleave="hovering; inHover = null">
                         <figure class="picture">
            <img  :alt="example.service ? example.service : alt" :src="example.img" :height="itemHeight" :max-height="itemHeight" :width="itemWidth" :max-width="itemWidth" class="mr-3 lazyload">
          </figure>
             
<!--               <v-img :alt="example.service ? example.service : alt" :src="example.img" :height="itemHeight" :max-height="itemHeight" :width="itemWidth" :max-width="itemWidth" class="mr-3 asddd"></v-img>
 -->              <v-chip v-if="example.tag && inHover !== i" :color="example.color" text-color="white" style="bottom: 20px; position: absolute; font-size: 15px;">{{example.tag}}</v-chip>
              <span class="hidden-box" v-if="isHovered && hoverInfo" :style="'background-color:'+example.color+'; color:'+example.textColor">
